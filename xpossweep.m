...
N = 512;
xp = 1:5:N-1;
spikedata = zeros(length(xp),length(freq),length(taut),length(w00),length(ampp),length(psi));
spikesem = zeros(length(xp),length(freq),length(taut),length(w00),length(ampp),length(psi));
angless = zeros(length(xp),length(freq),length(taut),length(w00),length(ampp),trials);
for px = 1:length(xp)
...
angless(px,fr,tt,ff,d,q) = psi(index);
end
spikedata(px,fr,tt,ff,d,:) = mean(nospike(:,:),2);
spikesem(px,fr,tt,ff,d,:) = std(nospike(:,:),0,2)/sss;
spikeanglem(px,fr,tt,ff,d)= mean(angless(px,fr,tt,ff,d,:));
spikeanglesem(px,fr,tt,ff,d) = std(angless(px,fr,tt,ff,d,:))/sss;
...
figure(1)
errorbar(xp,ang,angsem,'-o')
xlabel('Recurrent strength')
