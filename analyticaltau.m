...
for t = 3:length(timet)
   ...
   %a0(t) = a0(t-1) + (I0/tau + a1(t-1)*dz/dt- (1-lamb)/tau*a0(t-1))*dt;
    ...
    Rt = 1  + amp/(1-lamb)*exp(-(1-lamb)*t/tau) + amp/(2-lamb)*(exp(-(z(t-1)-z_0)^2/(8*a^2))  - exp(-z_0^2/(8*a^2))) + 0*sqrt(2)/(U_0*(1-lamb))*(1+amp)*(1 - exp(-(1-lamb)*t/tau))+ 0*1/(sqrt(2)*U_0*tau)*(sin(omega*t)/(omega) + cos(omega*t) + amp*sin(omega*t + pssi)/(omega)  + amp*cos(omega*t+pssi) - (exp(-(1-lamb)*t/tau)*(1 + sin(pssi)/(omega) + cos(pssi)/2))); 
    z(t) = z(t-1) + ((amp/tau)*(z_0-z(t-1))*exp(-(z(t-1) - z_0)^2/(8*a^2))+ a1(t-1)/(Rt))*dt;
    %z(t) = z(t-1) + ((2*a/tau)*(I1 + a1(t-1))/(sqrt(sqrt(2*pi)*a)*U_0 + a0(t-1)))*dt;
