...
for j=1:length(psi)
    check = 0;
m_inti = 0*ones(N,1) + (1e-1*cos(2*pi/N * (x-250)))';
m = zeros(N,T/dt);
s1 = zeros(N,T/dt);
s2 = zeros(N,T/dt);
sp1 = zeros(N,T/dt);
sp2 = zeros(N,T/dt);
m(:,1) = m_inti;
n_inti = 0*ones(N,1) + (1e-1*cos(2*pi/N * (x-200)))';
n = zeros(N,T/dt);
n(:,1) = n_inti;
%omega = 0.0628;
omega = freq(fr);
...
figure(10)
surf(s2','LineStyle','none'),view([0,90])
...
ylabel(h, 'tau (msec)')
%plot(psi,spd,'o','color',linecolors(fr,:))
plot(psi,spd,'o')
