...
for t = 2:length(timet)
    ...
    u1(:,t) = u1(:,t-1) + (1+1*cos(omega*t) + sw(t)*am*u2(:,t-1)+ p*(J(:,:)*r1(:,t-1)) - u1(:,t-1))*dt/tau;
    r1(:,t) = (u1(:,t).^2)/(1 + k*p*(sum(u1(:,t).^2)));
    u2(:,t) = u2(:,t-1) + ( 1+1*cos(omega*t + psi(g))+ p*(J(:,:)*r2(:,t-1)) - u2(:,t-1))*dt/tau;
...
surf(r1','LineStyle','none'),view([0,90])
set(gca,'fontsize',14), colormap(jet), colorbar
